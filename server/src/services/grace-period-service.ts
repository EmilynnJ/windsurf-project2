...
export class GracePeriodService {
  private static activeGracePeriods: Map<number, ActiveGracePeriod> = new Map();

  /**
   * Initiates a grace period for a reading after disconnection
   */
  static async initiateGracePeriod(readingId: number, userId: number): Promise<void> {
   ...
   */
  static clearGracePeriod(readingId: number): void {
   ...
   */
  static isInGracePeriod(readingId: number): boolean {
   ...
   */
  private static async handleGracePeriodExpiration(readingId: number, userId: number): Promise<void> {
    try {
   ...
   */
  static getRemainingGraceTime(readingId: number): number | null {
    const gracePeriod = this.activeGracePeriods.get(readingId);
    if (!gracePeriod) {
      return null;
    }
