...
export async function uploadProfileImage(userId: number, file: Express.Multer.File): Promise<string> {
  // Validate the image
  validateImage(file);
...
export async function updateProfileImage(userId: number, imageUrl: string): Promise<void> {
  const db = getDb();
...
export async function removeProfileImage(userId: number): Promise<void> {
  const db = getDb();
