...
// Send a message
router.post('/send', authMiddleware, messagingRateLimitMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (parentMessageId) {
      ...
      const isValidParent =
        (parentMessage.senderId === senderId && parentMessage.receiverId === receiverId) ||
...
router.get('/inbox', authenticateToken, async (req, res) => {
  try {
    ...
    const inboxData = await db
      .select({
        otherUserId: sql<number>`CASE WHEN ${messages.senderId} = ${userId} THEN ${messages.receiverId} ELSE ${messages.senderId} END`.as('other_user_id'),
        ...
        unreadCount: sql<number>`COUNT(CASE WHEN ${messages.readAt} IS NULL AND ${messages.senderId} != ${userId} THEN 1 END)`.as('unread_count'),
