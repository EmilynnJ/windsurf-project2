...
router.get('/readers/online', async (req, res, next) => {
  try {
...
router.get('/readers/:id', async (req, res, next) => {
  try {
    ...
    const { auth0Id, stripeCustomerId, stripeConnectId, accountBalance, totalEarnings, totalSpent, ...publicProfile } = reader;
...
// PUT /api/readers/:id/status — toggle online/offline (Reader only)
router.put(
  '/readers/:id/status',
  checkJwt,
  resolveUser,
...
// PUT /api/readers/:id/pricing — update per-minute rates (Reader only)
router.put(
  '/readers/:id/pricing',
  checkJwt,
  resolveUser,
...
// GET /api/users/:id/readings — user's reading history (Auth)
router.get(
  '/users/:id/readings',
  checkJwt,
  resolveUser,
  async (req, res, next) => {
    try {
      ...
      if (userId !== req.user!.id && req.user!.role !== 'admin') {
        throw new AppError(403, 'Can only view your own reading history');
      }
      ...
      const history = await db
        ...
        .from(readings)
        .where(eq(
          req.user!.role === 'reader' ? readings.readerId : readings.clientId,
          userId,
        ))
        .orderBy(desc(readings.createdAt))
        .limit(50);
...
// GET /api/users/:id/transactions — user's transaction history (Auth)
router.get(
  ...
  async (req, res, next) => {
    try {
      const userId = parseInt(req.params.id!, 10);
      if (userId !== req.user!.id && req.user!.role !== 'admin') {
        throw new AppError(403, 'Can only view your own transactions');
      }
