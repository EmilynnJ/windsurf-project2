...
router.get('/', async (req, res) => {
    try {
        ...
        // Apply filters
        const conditions = [eq(users.role, 'reader')];
        ...
        if (typeof validatedParams.minPrice === 'number') {
            conditions.push(or(gte(users.pricingChat, validatedParams.minPrice), gte(users.pricingVoice, validatedParams.minPrice), gte(users.pricingVideo, validatedParams.minPrice)));
        }
        if (typeof validatedParams.maxPrice === 'number') {
            conditions.push(or(lte(users.pricingChat, validatedParams.maxPrice), lte(users.pricingVoice, validatedParams.maxPrice), lte(users.pricingVideo, validatedParams.maxPrice)));
        }
...
router.patch('/me/online', authMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
...
router.patch('/me/pricing', authMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        ...
        if (typeof pricingUpdate.pricingChat === 'undefined' &&
