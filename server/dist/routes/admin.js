...
import { users, readings, transactions, forumPosts, forumComments, forumFlags } from '@soulseer/shared/schema';
...
const authMiddleware = rawAuthMiddleware;
...
router.get('/stats', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const balanceStats = await db.select({
            totalBalance: sql `SUM(${users.accountBalance})`,
            avgBalance: sql `AVG(${users.accountBalance})`,
            minBalance: sql `MIN(${users.accountBalance})`,
            maxBalance: sql `MAX(${users.accountBalance})`,
        }).from(users);
        ...
        const financialStats = await db.select({
            totalRevenue: sql `SUM(CASE WHEN ${transactions.amount} > 0 THEN ${transactions.amount} ELSE 0 END)`,
            totalPayouts: sql `SUM(CASE WHEN ${transactions.amount} < 0 THEN ABS(${transactions.amount}) ELSE 0 END)`,
...
router.patch('/users/bulk-update', authMiddleware, adminMiddleware, async (req, res) => {
    try {
...
router.delete('/users/bulk-delete', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const adminUsers = await db.select()
            .from(users)
            .where(and(eq(users.role, 'admin'), sql `${users.id} = ANY(${validatedData.userIds})`));
        ...
        await db.transaction(async (tx) => {
            ...
            await tx.delete(users).where(sql `${users.id} = ANY(${validatedData.userIds})`);
        });
...
router.get('/users/search', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        if (validatedParams.q) {
            conditions.push(or(ilike(users.username, `%${validatedParams.q}%`), ilike(users.fullName, `%${validatedParams.q}%`), ilike(users.email, `%${validatedParams.q}%`)));
        }
...
router.get('/readings', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const conditions = [];
        if (status) {
            conditions.push(eq(readings.status, status));
        }
        if (type) {
...
router.get('/forum/flags', authMiddleware, adminMiddleware, async (req, res) => {
    try {
...
router.post('/forum/moderate', authMiddleware, adminMiddleware, async (req, res) => {
    try {
...
router.post('/payouts/:readerId', authMiddleware, adminMiddleware, async (req, res) => {
    try {
