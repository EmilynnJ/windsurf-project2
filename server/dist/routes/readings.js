...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        if (type === 'chat')
            pricePerMinute = reader.pricingChat;
        if (type === 'voice')
            pricePerMinute = reader.pricingVoice;
        if (type === 'video')
            pricePerMinute = reader.pricingVideo;
    ...
    try {
        ...
        try {
            ...
            if (readingRecord?.resourceId && readingRecord?.sid) {
                await stopRecording(readingRecord.channelName, String(req.user.id), readingRecord.resourceId, readingRecord.sid);
            }
        }
        catch (recordingError) {
...
router.get('/reader/:readerId', authMiddleware, async (req, res) => {
    try {
