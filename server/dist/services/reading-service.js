...
export class ReadingService {
     ...
     */
    static async acceptReading(readingId, readerId) {
     ...
     */
    static async startReading(readingId, userId) {
        const reading = await db.query.readings.findFirst({
            where: eq(readings.id, readingId),
        });
        if (!reading) {
     ...
     */
    static async addMessage(readingId, senderId, content) {
        const reading = await db.query.readings.findFirst({
            where: eq(readings.id, readingId),
        });
        if (!reading) {
        ...
        let currentTranscript = reading.chatTranscript;
     ...
     */
    static async endReading(readingId, userId) {
        const reading = await db.query.readings.findFirst({
            where: eq(readings.id, readingId),
        });
        if (!reading) {
     ...
     */
    static async cancelReading(readingId, userId) {
        const reading = await db.query.readings.findFirst({
            where: eq(readings.id, readingId),
        });
        if (!reading) {
     ...
     */
    static async updateReadingStatus(readingId, status) {
     ...
     */
    static async handleReconnection(readingId, userId) {
        const reading = await db.query.readings.findFirst({
            where: eq(readings.id, readingId),
        });
        if (!reading) {
     ...
     */
    static async getUserReadings(userId, limit = 20, offset = 0) {
     ...
     */
    static async getReaderReadings(readerId, status) {
        const userReadings = await db.query.readings.findMany({
            where: status
     ...
     */
    static async getClientReadings(clientId, status) {
        const userReadings = await db.query.readings.findMany({
            where: status
     ...
     */
    static async generateAgoraToken(readingId, userId, role = 'publisher') {
        const reading = await db.query.readings.findFirst({
            where: eq(readings.id, readingId),
        });
        if (!reading) {
     ...
     */
    static mapToReadingSession(dbReading) {
        return {
            id: dbReading.id,
            ...
            chatTranscript: dbReading.chatTranscript,
        };
    }
}
